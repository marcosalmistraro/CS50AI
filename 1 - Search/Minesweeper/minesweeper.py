...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # Mark cell both as move made and safe
        self.moves_made.add(cell)
        self.mark_safe(cell)

        # Add neighboring cells on the board to the knowledge base
        new_sentence_cells = set()

        all_neighboring_cells = {(i, j)
                                 for i in range(cell[0] - 1, cell[0] + 2)
                                 for j in range(cell[1] - 1, cell[1] + 2)}
        ...
        neighboring_cells = all_neighboring_cells.intersection(board_cells)
        neighboring_cells = neighboring_cells.difference({cell})

        # Loop over neighboring_cells and add meaningful cells to the sentence
        ...
        # Append new sentence to the knowledge base
        self.knowledge.append(Sentence(new_sentence_cells, count))

        # Infer new sentences iteratively, when possible
        ...
        while knowledge_updated:
            ...
            for sentence_1 in self.knowledge:
                for sentence_2 in self.knowledge:
                    ...
                    # Sentences with the same information are ignored
                    if sentence_1.cells == sentence_2.cells:
                        continue

                    if sentence_1.cells.issubset(sentence_2.cells):
                        new_sentence_cells = sentence_2.cells.difference(sentence_1.cells)
        ...
        for i in range(self.width):
            for j in range(self.height):
                ...
                if (cell not in self.moves_made) and (cell not in self.mines):
                    rand_cell_list.append(cell)
        if not rand_cell_list:
            return None
        return random.choice(rand_cell_list)
